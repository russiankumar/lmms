...
void StepRecorder::noteReleased(const Note & n)
{
	...
	if(stepNote != nullptr && stepNote->isPressed())
	{
		...
		//check if all note are released, apply notes to clip(or dimiss if length is zero) and prepare to record next step
...
void StepRecorder::setCurrentClip( Clip* newClip )
{
	if(m_clip != NULL && m_clip != newClip)
	{
		dismissStep();
	}
