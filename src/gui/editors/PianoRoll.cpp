...
void PianoRoll::setGhostClip( Clip* newClip )
{
	// Expects a pointer to a clip or nullptr.
	m_ghostNotes.clear();
	if( newClip != nullptr )
	{
		for( Note *note : newClip->notes() )
		{
...
void PianoRoll::setCurrentClip( Clip* newClip )
{
	if( hasValidClip() )
	{
	...
	m_stepRecorder.setCurrentClip(newClip);
	...
	connect( m_clip, SIGNAL( destroyedClip( Clip* ) ), this, SLOT( hideClip( Clip* ) ) );
...
void PianoRoll::hideClip( Clip* clip )
{
	if( m_clip == clip )
	{
		setCurrentClip( NULL );
...
void PianoRoll::keyPressEvent(QKeyEvent* ke)
{
	...
	if( hasValidClip() && ke->modifiers() == Qt::NoModifier )
	{
		...
		case Qt::Key_Left:
			{
				...
				else if( ke->modifiers() & Qt::AltModifier)
				{
					...
					if (hasValidClip())
					{
						Clip * p = direction > 0 ? m_clip->nextClip()
										: m_clip->previousClip();
						if(p != NULL)
						{
							setCurrentClip(p);
...
void PianoRoll::keyReleaseEvent(QKeyEvent* ke )
{
	...
	switch( ke->key() )
	{
		...
		case Qt::Key_Z:
		case Qt::Key_R:
			if( hasValidClip() && ke->modifiers() == Qt::ControlModifier )
			{
				update();
			}
			break;
...
void PianoRoll::paintEvent(QPaintEvent * pe )
{
	...
	int l = ( hasValidClip() )? (int) m_clip->length() : 0;
	...
	if(hasValidClip() && gui->pianoRoll()->hasFocus())
	{
...
{
	return hasValidClip() && noteUnderMouse() != NULL;
}
	...
	m_editor(new PianoRoll())
{
	...
	// Connections
	connect( m_editor, SIGNAL( currentClipChanged() ), this, SIGNAL( currentClipChanged() ) );
	connect( m_editor, SIGNAL( currentClipChanged() ), this, SLOT( updateAfterClipChange() ) );
}




const Clip* PianoRollWindow::currentClip() const
{
...
void PianoRollWindow::setGhostClip( Clip* clip )
{
...
void PianoRollWindow::setCurrentClip( Clip* clip )
{
	...
	if ( clip )
	{
		...
		connect( clip->instrumentTrack(), SIGNAL( nameChanged() ), this, SLOT( updateAfterClipChange()) );
		connect( clip, SIGNAL( dataChanged() ), this, SLOT( updateAfterClipChange() ) );
	}
	else
	{
...
void PianoRollWindow::updateAfterClipChange()
{
	patternRenamed();
...
void PianoRollWindow::patternRenamed()
{
	if ( currentClip() )
	{
		setWindowTitle( tr( "Piano-Roll - %1" ).arg( currentClip()->name() ) );
	}
	else
	{
		setWindowTitle( tr( "Piano-Roll - no clip" ) );
